...
def get_batch(batch_size=64, iter=0, file_dir):
    batch_imgs = np.zeros([batch_size, IMG_WIDTH*IMG_HEIGHT])
    files_paths = os.listdir(file_dir)
    for i in range(batch_size):
        index = batch_size*iter + i
        img_name = files_paths[index]
...
file_dir = "./cap1/test/"
files_paths = os.listdir(file_dir)
for iter in range(len(files_paths) / batch_size):
    batch_imgs = get_batch(iter, files_paths)  #load data
    gen_imgs = sess.run([batch_imgs])
    prediction = sess.run([net_predict],feed_dict={
            net_input : gen_imgs})
    # prediction=[array([[57, 32, 43, 25, 61, 61, 60, 58]])]
    max_index = prediction[0][0]
    max_chars = [label_to_char[i] for i in max_index]
    max_chars = "".join(max_chars)
    print(max_chars)
    # if (max_chars == p.split('_')[1].split('.')[0]):
    #     right += 1
    answer_file.writelines(p + ":" + str(max_chars) + '\n')
# print "test accuracy:" + str(float(right)/len(files_paths))#!
# answer_file.writelines("test accuracy:" + str(float(right)/len(files_paths)) + '\n')
